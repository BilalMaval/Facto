...
export function CodeAvailabilityHint({ status, label = 'Worker ID' }: { status: CodeStatus; label?: string }) {
  ...
  if (status === 'taken') return <p className="mt-1 text-xs text-red-600">This {label} is already in use</p>
  return null
}
