-- A shortened week spanning a Week Start Day change (see resolveWeekBounds
-- in lib/dates.ts) never reaches its real day off — it's cut short before
-- ever getting there, so every day in it is a genuine working day. The
-- previous version of this function always treated p_week_end as the day
-- off (excluded from the 6-day present/absent/half-day sum, eligible for
-- holiday_wage instead) regardless of week length, which wrongly zeroed out
-- pay for the last day of any such shortened week. Now that exclusion only
-- applies to a full 7-day week (p_week_end - p_week_start = 6); a shorter
-- week counts every day toward the regular sum and never applies
-- holiday_wage, since it has no holiday day to apply it to.
...
begin
  ...
  v_has_day_off := (p_week_end - p_week_start) = 6;

  select
      count(*),
      coalesce(sum(case when v_has_day_off and attendance_date = p_week_end then 0
        else case status when 'present' then 1 when 'half_day' then 0.5 else 0 end
      end), 0),
      ...
      coalesce(sum(case when v_has_day_off and attendance_date = p_week_end and status = 'present' then holiday_wage else 0 end), 0)
