...
export default async function AdminSupportPage({
  ...
  const { data: tickets } = await supabase
    ...
    .select('id, subject, status, created_at, updated_at, organization:organizations(name)')
    .order('updated_at', { ascending: false })

  const ticketIds = (tickets ?? []).map((t) => t.id)
  const { data: messages } = ticketIds.length
    ? await supabase.from('support_ticket_messages').select('ticket_id, body').in('ticket_id', ticketIds)
    : { data: [] as { ticket_id: string; body: string }[] }
  ...
  let rows = (tickets ?? []).map((t) => {
    ...
    return { ...t, orgName: org?.name ?? '—', messageText: bodiesByTicket.get(t.id) ?? '' }
  })
